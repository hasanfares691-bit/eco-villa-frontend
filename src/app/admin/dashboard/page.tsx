...
export default function AdminDashboard() {
  ...
  const fetchData = async (token: string) => {
    ...
    try {
      // تمرير كائن فارغ لتجنب خطأ التايب سكريبت
  ...
  // ✅ الإجبار الصريح لمعالجة الخانات المفقودة
  ...
  return (
    <div dir="rtl" className="max-w-md mx-auto min-h-screen bg-[#F8F9FA] relative font-sans overflow-x-hidden shadow-2xl border-x border-gray-200 flex flex-col">
      <header className="bg-[#1E1E2D] text-white pt-10 pb-0 shadow-lg relative z-20 shrink-0">
        <div className="px-6 flex justify-between items-center mb-6">
          <div><h1 className="text-2xl font-black text-[#7CB342] leading-none tracking-tight">Eco Villa</h1><span className="text-gray-400 text-[10px] font-bold uppercase tracking-widest">Workspace</span></div>
          <button onClick={() => { localStorage.removeItem('eco_villa_token'); router.push('/'); }} className="flex items-center gap-1.5 bg-red-500/10 text-red-400 px-3 py-2 rounded-xl text-xs font-bold hover:bg-red-500 hover:text-white transition active:scale-95"><LogoutIcon /> خروج</button>
        </div>
        <div className="flex overflow-x-auto gap-2 px-4 pb-4 no-scrollbar">
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => tab.id === 'bookings' ? router.push('/admin/bookings') : setActiveView(tab.id)} className={`flex items-center gap-2 whitespace-nowrap px-4 py-2.5 rounded-xl text-sm font-bold transition-all shadow-sm ${activeView === tab.id ? 'bg-blue-600 text-white' : 'bg-white/10 text-gray-300 hover:bg-white/20'}`}>
      ...
      <main className="flex-1 overflow-y-auto p-5 pb-20">
        <div className="flex justify-between items-center mb-6">
           <h2 className="text-xl font-black text-gray-800">{tabs.find(t => t.id === activeView)?.label} {tabs.find(t => t.id === activeView)?.icon}</h2>
        </div>

        {activeView === 'home' && (
          <div className="space-y-4">
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 text-center mb-4"><h3 className="text-xl font-black text-gray-800 mb-1">أهلاً بك في غرفة العمليات</h3><p className="text-gray-500 text-sm font-medium">اختر القسم الذي تريد إدارته من القائمة أدناه</p></motion.div>
            <div className="grid grid-cols-2 gap-4">
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => router.push('/admin/bookings')} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center gap-3 hover:shadow-md transition"><div className="w-16 h-16 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center text-3xl">📡</div><div><h3 className="text-sm font-black text-gray-800">الرادار</h3><p className="text-[10px] text-gray-500 font-bold mt-1">إدارة الحجوزات</p></div></motion.button>
              <motion.button whileTap={{ scale: 0.95 }} onClick={() => setActiveView('farms')} className="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 flex flex-col items-center justify-center gap-3 hover:shadow-md transition"><div className="w-16 h-16 bg-green-50 text-green-600 rounded-full flex items-center justify-center text-3xl">🏡</div><div><h3 className="text-sm font-black text-gray-800">المزارع</h3><p className="text-[10px] text-gray-500 font-bold mt-1">إضافة وتعديل</p></div></motion.button>
            </div>
          </div>
        )}
        ...
        {activeView === 'farms' && (
          <div className="space-y-4">
            <button onClick={openAddNewFarm} className="w-full py-4 bg-green-600 text-white rounded-2xl font-black shadow-lg hover:bg-green-700 transition active:scale-95 flex items-center justify-center gap-2 mb-4">+ إضافة مزرعة جديدة</button>
            {farmsList.length === 0 ? ( <div className="text-center p-10 text-gray-400 font-bold bg-white rounded-3xl border border-gray-100">لا يوجد مزارع مضافة بعد.</div> ) : (
              farmsList.map((f, i) => (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} key={i} className={`bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col ${f.status === 'inactive' ? 'opacity-60 grayscale' : ''}`}>
                  <div className="flex gap-4 items-center">
                    ...
                    <div className="flex-1">
                       <div className="flex justify-between items-start"><span className="text-xs font-bold text-gray-400 uppercase">{f.admin_code || 'EV_---'}</span><span className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${f.status === 'inactive' ? 'bg-red-50 text-red-600' : 'bg-blue-50 text-blue-600'}`}>{f.status === 'inactive' ? 'موقوفة' : 'نشط'}</span></div>
                       <h3 className="font-bold text-gray-800 mt-1 leading-tight">{f.name}</h3><p className="text-xs font-black text-gray-800 mt-1">{f.price_weekday?.toLocaleString()} ل.س</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 mt-4 pt-3 border-t border-gray-50">
                     <button onClick={() => handleEditFarmClick(f)} className="py-2.5 bg-blue-50 text-blue-600 rounded-xl text-xs font-bold active:scale-95 transition hover:bg-blue-100">✏️ تعديل</button>
                     {/* زر التقييمات الجديد */}
                     <button onClick={() => handleOpenReviews(f)} className="py-2.5 bg-yellow-50 text-yellow-700 rounded-xl text-xs font-bold active:scale-95 transition hover:bg-yellow-100">⭐️ التقييمات</button>
                     <button onClick={() => handleDeleteFarmClick(f.id)} className="py-2.5 bg-red-50 text-red-600 rounded-xl text-xs font-bold active:scale-95 transition hover:bg-red-100">🗑️ إيقاف</button>
                  </div>
                </motion.div>
              ))
            )}
        ...
        {activeView !== 'farms' && activeView !== 'home' && (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-white rounded-3xl shadow-sm border border-gray-100 p-12 text-center mt-4">
            <div className="text-5xl mb-4 opacity-30 animate-pulse">🚧</div><h3 className="text-xl font-black text-gray-800 mb-2">قيد التطوير</h3><p className="text-gray-500 font-medium text-sm">سيتم برمجة هذا القسم قريباً.</p>
          </motion.div>
        )}
      </main>

      {/* 🔴 مودال إدارة التقييمات */}
      <AnimatePresence>
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6">
                ...
                <div className={`p-5 rounded-2xl shadow-sm border ${isEditingReview ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                  <div className="flex justify-between items-center mb-3">
                    <h4 className="text-sm font-bold text-gray-800">{isEditingReview ? '✏️ تعديل التقييم' : '➕ إضافة تقييم يدوي'}</h4>
      ...
      <AnimatePresence>
        {showAddFarmModal && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-black/60 backdrop-blur-sm z-40" />
            <motion.div initial={{ opacity: 0, y: "100%" }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: "100%" }} transition={{ type: "spring", bounce: 0, duration: 0.4 }} className="absolute top-10 bottom-0 left-0 right-0 bg-[#F8F9FA] rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.2)] z-50 overflow-hidden flex flex-col">
              <div className="bg-white p-5 border-b border-gray-200 flex justify-between items-center shrink-0">
              ...
              <div className="flex-1 overflow-y-auto p-5 pb-24">
                {newFarmOwnerCredentials ? (
                  ...
                  <form onSubmit={handleAddFarm} className="space-y-6">
                    {!isEditingFarm && (
                      <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4"><h4 className="text-sm font-black text-blue-800 mb-2 flex items-center gap-2"><span className="text-lg">🧑‍🌾</span> بيانات المالك (الأساسية)</h4><div><label className="text-xs font-bold text-gray-600 mb-1 block">اسم المالك الرباعي</label><input required value={farmData.ownerName} onChange={e=>setFarmData({...farmData, ownerName: e.target.value})} type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900 focus:border-blue-500" placeholder="مثال: محمد الفارس" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">رقم الموبايل (حساب الدخول)</label><input required value={farmData.ownerPhone} onChange={e=>setFarmData({...farmData, ownerPhone: e.target.value})} type="text" dir="ltr" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900 focus:border-blue-500" placeholder="09xxxxxxx" /></div><div><label className="text-xs font-bold text-red-600 mb-1 block">كلمة المرور للمالك (تعيين يدوي)</label><input required value={farmData.ownerPassword} onChange={e=>setFarmData({...farmData, ownerPassword: e.target.value})} type="text" dir="ltr" className="w-full bg-red-50 border border-red-200 rounded-xl p-3.5 text-sm font-bold text-gray-900 focus:border-red-500" placeholder="مثال: pass1234" /></div></div>
                    )}
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4"><h4 className="text-sm font-black text-gray-800 mb-2 flex items-center gap-2"><span className="text-lg">🏡</span> معلومات المزرعة والمواصفات</h4><div><label className="text-xs font-bold text-gray-600 mb-1 block">كود الإدارة (ID)</label><input required value={farmData.adminCode} onChange={e=>setFarmData({...farmData, adminCode: e.target.value})} type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-blue-600" placeholder="EV_151" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">اسم المزرعة التجاري</label><input required value={farmData.name} onChange={e=>setFarmData({...farmData, name: e.target.value})} type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="مزرعة السعادة ريزورت" /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-600 mb-1 block">المساحة (متر مربع)</label><input required value={farmData.areaSize} onChange={e=>setFarmData({...farmData, areaSize: e.target.value})} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="2000" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">عدد الغرف</label><input required value={farmData.roomsCount} onChange={e=>setFarmData({...farmData, roomsCount: e.target.value})} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="3" /></div></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-600 mb-1 block">أقصى استيعاب</label><input required value={farmData.maxCapacity} onChange={e=>setFarmData({...farmData, maxCapacity: e.target.value})} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="15" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">نوع الضيوف</label><select value={farmData.guestType} onChange={e=>setFarmData({...farmData, guestType: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900"><option value="families">عائلات فقط</option><option value="mixed">عائلات وشباب</option></select></div></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">الوصف الدقيق والميزات</label><textarea required={!isEditingFarm} value={farmData.description} onChange={e=>setFarmData({...farmData, description: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-sm font-bold text-gray-900 h-28" placeholder="اكتب المواصفات بدقة هنا..."></textarea></div></div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4"><h4 className="text-sm font-black text-gray-800 mb-2 flex items-center gap-2"><span className="text-lg">💰</span> الموقع والتسعير</h4><div><label className="text-xs font-bold text-gray-600 mb-1 block">المنطقة الجغرافية</label><input required value={farmData.region} onChange={e=>setFarmData({...farmData, region: e.target.value})} type="text" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="يعفور" /></div><div className="grid grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-600 mb-1 block">سعر (عادي)</label><input required value={farmData.priceWeekday} onChange={e=>setFarmData({...farmData, priceWeekday: e.target.value})} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="500000" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">سعر (عطلة)</label><input required value={farmData.priceWeekend} onChange={e=>setFarmData({...farmData, priceWeekend: e.target.value})} type="number" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-orange-600" placeholder="750000" /></div></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">التصنيف الاختياري</label><select value={farmData.classification} onChange={e=>setFarmData({...farmData, classification: e.target.value})} className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-purple-700"><option value="">بدون تصنيف</option><option value="الأكثر تفضيلاً 🌟">الأكثر تفضيلاً 🌟</option><option value="عروض اللحظة ⏳">عروض اللحظة ⏳</option></select></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">رابط خرائط غوغل</label><input value={farmData.googleMapsUrl} onChange={e=>setFarmData({...farmData, googleMapsUrl: e.target.value})} type="url" dir="ltr" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="https://maps.google.com/..." /></div></div>
                    <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col gap-4"><h4 className="text-sm font-black text-gray-800 mb-2 flex items-center gap-2"><span className="text-lg">🔗</span> الوسائط</h4><div><label className="text-xs font-bold text-gray-600 mb-1 block">روابط الصور (مفصولة بفاصلة ,)</label><textarea value={farmData.imageUrls} onChange={e=>setFarmData({...farmData, imageUrls: e.target.value})} dir="ltr" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs font-bold text-gray-900 h-24 font-mono leading-relaxed" placeholder="http://image1.jpg,
http://image2.jpg..."></textarea></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">رابط التلغرام</label><input value={farmData.telegramVideoUrl} onChange={e=>setFarmData({...farmData, telegramVideoUrl: e.target.value})} type="url" dir="ltr" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="https://t.me/ecovilla/12" /></div><div><label className="text-xs font-bold text-gray-600 mb-1 block">أرقام التواصل</label><input required={!isEditingFarm} value={farmData.contactNumbers} onChange={e=>setFarmData({...farmData, contactNumbers: e.target.value})} type="text" dir="ltr" className="w-full bg-gray-50 border border-gray-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" placeholder="09xxxx, 09yyyy" /></div></div>
                    <div className="bg-orange-50 p-5 rounded-2xl border border-orange-100 flex flex-col gap-4"><h4 className="text-sm font-black text-orange-800 mb-2 flex items-center gap-2"><span className="text-lg">🕵️‍♂️</span> قسم الإدارة السري</h4><div><label className="text-xs font-bold text-orange-600 block mb-1">تاريخ آخر فيديو تحقق</label><input type="date" required={!isEditingFarm} value={farmData.lastVerificationDate} onChange={e=>setFarmData({...farmData, lastVerificationDate: e.target.value})} className="w-full bg-white border border-orange-200 rounded-xl p-3.5 text-sm font-bold text-gray-900" /></div><div><label className="text-xs font-bold text-orange-600 block mb-1">ملاحظات سرية لفريق العمل</label><textarea value={farmData.adminNotes} onChange={e=>setFarmData({...farmData, adminNotes: e.target.value})} className="w-full bg-white border border-orange-200 rounded-xl p-4 text-sm font-bold text-gray-900 h-24" placeholder="مثال: المالك يتأخر بالرد..."></textarea></div></div>
                  </form>
                )}
              </div>
